//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//		
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

using RMA.Common.Database.Contracts.Repository;
using RMA.Service.Admin.MasterDataManager.Contracts.Enums;

namespace RMA.Service.Billing.Database.Entities
{
    public partial class payment_Allocation : IAuditableEntity, ISoftDeleteEntity, IEntityStatus
    {
        public int AllocationId { get; set; } // AllocationId (Primary key)
        public int? PaymentId { get; set; } // PaymentId
        public int PayeeId { get; set; } // PayeeId
        public int AllocationStatusId { get; set; } // AllocationStatusId
        public int? ClaimInvoiceAllocationId { get; set; } // ClaimInvoiceAllocationId
        public int? MedicalInvoiceId { get; set; } // MedicalInvoiceId
        public int? DaysOffInvoiceId { get; set; } // DaysOffInvoiceId
        public int? MonthlyPensionLedgerId { get; set; } // MonthlyPensionLedgerId
        public int? CommutationId { get; set; } // CommutationId
        public int? OverPaymentId { get; set; } // OverPaymentId
        public int? PdAwardId { get; set; } // PDAwardId
        public int? PostRetirementMedicalAnnuityInvoiceHeaderId { get; set; } // PostRetirementMedicalAnnuityInvoiceHeaderId
        public decimal AssessedAmount { get; set; } // AssessedAmount
        public decimal AssessedVat { get; set; } // AssessedVAT
        public decimal? AssessedAmountInclusive { get; private set; } // AssessedAmountInclusive
        public bool IsActive { get; set; } // IsActive
        public bool IsDeleted { get; set; } // IsDeleted
        public string CreatedBy { get; set; } // CreatedBy (length: 50)
        public System.DateTime CreatedDate { get; set; } // CreatedDate
        public string ModifiedBy { get; set; } // ModifiedBy (length: 50)
        public System.DateTime ModifiedDate { get; set; } // ModifiedDate
        public PaymentTypeEnum? PaymentType { get; set; } // PaymentTypeId
        public string PensionCaseNumber { get; set; } // PensionCaseNumber (length: 50)
        public string BenefitCode { get; set; } // BenefitCode (length: 50)
        public int? LedgerId { get; set; } // LedgerId

        // Foreign keys

        /// <summary>
        /// Parent client_RolePlayer pointed by [Allocation].([PayeeId]) (FK_Allocation_RolePlayer)
        /// </summary>
        public virtual client_RolePlayer RolePlayer { get; set; } // FK_Allocation_RolePlayer

        /// <summary>
        /// Parent payment_Payment pointed by [Allocation].([PaymentId]) (FK_Allocation_Payment)
        /// </summary>
        public virtual payment_Payment Payment { get; set; } // FK_Allocation_Payment

        /// <summary>
        /// Parent policy_PostRetirementMedicalAnnuityInvoiceHeader pointed by [Allocation].([PostRetirementMedicalAnnuityInvoiceHeaderId]) (FK_Allocation_PostRetirementMedicalAnnuityInvoiceHeader)
        /// </summary>
        public virtual policy_PostRetirementMedicalAnnuityInvoiceHeader PostRetirementMedicalAnnuityInvoiceHeader { get; set; } // FK_Allocation_PostRetirementMedicalAnnuityInvoiceHeader

        public payment_Allocation()
        {
            IsActive = true;
            IsDeleted = false;
            CreatedDate = System.DateTime.Now;
            InitializePartial();
        }

        partial void InitializePartial();
    }

}
