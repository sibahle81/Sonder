...
namespace RMA.Service.ClaimCare.RuleTasks.MedicalInvoice.ClaimLiabilityStatus
{
    public class ClaimLiabilityStatusRule : IRule
    {
        public const string RuleName = Constants.LiabilityStatusRuleName;
        public string Name { get; } = RuleName;
        public string Code { get; } = "MI01";
        ...
        private RuleResult CreateResult(string ruleData)
        {
            ...
            {
                RuleName = RuleName,
                Passed = ruleData == Constants.Accepted || ruleData == Constants.MedicalLiability,
        ...
        {
            if (!result.Passed)
                result.MessageList.Add("Claim liability status must be Accepted or Medical Liability");
        }
    }
}
