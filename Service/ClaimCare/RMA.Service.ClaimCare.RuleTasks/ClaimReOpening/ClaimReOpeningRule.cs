...
{
    public class ClaimReOpeningRule : IRule
    {
        public const string RuleName = Constants.PreAuthClaimReOpen;
        public string Name { get; } = RuleName;
        public string Code { get; } = "PAC05";
        ...
        {
            if (ruleData.EventDate == DateTime.MinValue)
            {
                throw new BusinessException("Event date is invalid");
            }
            if (ruleData.RequestDate == DateTime.MinValue)
            {
        ...
        {
            var claimOlderThan2Years = ruleData.RequestDate <= ruleData.EventDate.AddYears(2);

            var result = new RuleResult
            {
                RuleName = RuleName,
                Passed = !claimOlderThan2Years && !ruleData.HasActiveTreatments && !ruleData.HasActiveInvoices && !ruleData.HasActiveMedicalReportForms,
        ...
        {
            if (!result.Passed)
                result.MessageList.Add("Claim should be re-opened");
        }
    }
}
