...
{
    public class VoluntaryFuneralAgeOptionFourRule : IRule
    {
        public const string RuleName = "Voluntary Fatal Cover Option 4 Age";
        public string Name { get; } = RuleName;
        public string Code { get; } = "FUNERAL04";
        ...
        public RuleResult Execute(IRuleContext context)
        {
            try
            {
                ...
                if (ruleData.ClientType != ClientTypeEnum.Individual)
                {
                    return new RuleResult
                    {
                        RuleName = RuleName,
                        Passed = true,
                        MessageList = new List<string> { "Voluntary Fatal Cover Age applies to individuals only" }
                    };
                }
        ...
        {
            if (age < MinimumAge)
                result.MessageList.Add("The minimum age is 18");

            if (age > MaximumAge)
                result.MessageList.Add("The maximum age is 85");
        }
