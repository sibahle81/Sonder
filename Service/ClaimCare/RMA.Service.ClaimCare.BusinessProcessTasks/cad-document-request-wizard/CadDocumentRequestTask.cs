...
{
    public class CadDocumentRequestTask : IWizardProcess
    {
        private readonly IEventService _eventService;

        public CadDocumentRequestTask(IEventService eventService)
        {
        ...
        public async Task<int> StartWizard(IWizardContext context)
        {
            ...
            var request = context.Deserialize<ClaimDocumentRequest>(context.Data);

            var personEvent = await _eventService.GetPersonEvent(request.PersonEventId);

            var label = $"Documents Requested by {request.RequestedBy}: PEV: {personEvent.PersonEventReferenceNumber} ";
            var stepData = new ArrayList() { request };
