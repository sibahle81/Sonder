...
{
    public class payment_PaymentConfiguration : System.Data.Entity.ModelConfiguration.EntityTypeConfiguration<payment_Payment>
    {
        public payment_PaymentConfiguration()
            : this("payment")
        {
        }

        public payment_PaymentConfiguration(string schema)
        {
            ToTable("Payment", schema);
            HasKey(x => x.PaymentId);

            Property(x => x.PaymentId).HasColumnName(@"PaymentId").HasColumnType("int").IsRequired().HasDatabaseGeneratedOption(System.ComponentModel.DataAnnotations.Schema.DatabaseGeneratedOption.Identity);
            ...
            Property(x => x.ErrorDescription).HasColumnName(@"ErrorDescription").HasColumnType("varchar(max)").IsOptional().IsUnicode(false);
            ...
            Property(x => x.StrikeDate).HasColumnName(@"StrikeDate").HasColumnType("datetime").IsOptional();
            Property(x => x.IsForex).HasColumnName(@"IsForex").HasColumnType("bit").IsOptional();
            Property(x => x.DestinationCountryId).HasColumnName(@"DestinationCountryId").HasColumnType("int").IsOptional();
            Property(x => x.Currency).HasColumnName(@"Currency").HasColumnType("nvarchar").IsOptional().HasMaxLength(50);
            Property(x => x.PayeeId).HasColumnName(@"PayeeId").HasColumnType("int").IsOptional();
            Property(x => x.BankReference).HasColumnName(@"BankReference").HasColumnType("varchar").IsOptional().IsUnicode(false).HasMaxLength(100);
            Property(x => x.PaymentMethod).HasColumnName(@"PaymentMethodId").HasColumnType("int").IsOptional();
            Property(x => x.IsReversed).HasColumnName(@"IsReversed").HasColumnType("bit").IsRequired();

            // Foreign keys
            HasOptional(a => a.PaymentInstruction).WithMany(b => b.Payments).HasForeignKey(c => c.PaymentInstructionId).WillCascadeOnDelete(false); // FK_Payment_PaymentInstruction
            HasOptional(a => a.PaymentSubmissionBatch).WithMany(b => b.Payments).HasForeignKey(c => c.PaymentSubmissonBatchid).WillCascadeOnDelete(false); // FK_Payment_PaymentSubmissionBatch
            HasOptional(a => a.RefundHeader).WithMany(b => b.Payments).HasForeignKey(c => c.RefundHeaderId).WillCascadeOnDelete(false); // FK_Payment_RefundHeader
        }
    }
