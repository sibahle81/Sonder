...
{
    public class AuthToDateNotAfterDateOfDeathRule : IRule
    {
        public const string RuleName = Constants.PreAuthDateOfDeathRuleName;
        public string Name { get; } = RuleName;
        public string Code { get; } = "PAC09";
        ...
        private RuleResult CreateResult(RuleData ruleData)
        {
            var result = new RuleResult
            {
                ...
                Passed = (Convert.ToDateTime(ruleData.DateAuthorisedTo).Date <= Convert.ToDateTime(ruleData.DateOfDeath).Date),
        ...
        {
            if (!result.Passed)
                result.MessageList.Add(Constants.PreAuthToDateDODValidation);
        }
