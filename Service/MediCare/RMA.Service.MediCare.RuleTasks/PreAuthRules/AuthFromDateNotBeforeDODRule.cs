...
{
    public class AuthFromDateAfterDateOfDeathRule : IRule
    {
        public const string RuleName = Constants.PreAuthDateOfDeathRuleName;
        public string Name { get; } = RuleName;
        public string Code { get; } = "PAC08";
        ...
        private RuleResult CreateResult(RuleData ruleData)
        {
            ...
            {
                RuleName = Constants.PreAuthDateOfDeathRuleName,
                Passed = (Convert.ToDateTime(ruleData.DateAuthorisedFrom).Date <= Convert.ToDateTime(ruleData.DateOfDeath).Date),
        ...
        {
            if (!result.Passed)
                result.MessageList.Add(Constants.PreAuthFromDateDODValidation);
        }
