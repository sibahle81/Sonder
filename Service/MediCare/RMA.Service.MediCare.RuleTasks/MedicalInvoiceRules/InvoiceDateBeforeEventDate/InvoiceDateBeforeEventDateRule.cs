...
{
    public class InvoiceDateBeforeEventDateRule : IRule
    {
        public const string RuleName = Constants.MedicalInvoiceEventDateRuleName;
        public string Name { get; } = RuleName;
        public string Code { get; } = "MI06";
        ...
        private RuleResult CreateResult(RuleData ruleData)
        {
            var result = new RuleResult
            {
                ...
                Passed = (Convert.ToDateTime(ruleData.InvoiceDate).Date >= Convert.ToDateTime(ruleData.EventDate).Date),
        ...
        {
            if (!result.Passed)
                result.MessageList.Add(Constants.InvoiceDateEventDateValidation);
        }
    }
}
