...
{
    public class TreatmentToDateBeforeEventDateRule : IRule
    {
        public const string RuleName = Constants.MedicalInvoiceEventDateRuleName;
        public string Name { get; } = RuleName;
        public string Code { get; } = "MI05";
        ...
        private RuleResult CreateResult(RuleData ruleData)
        {
            var result = new RuleResult
            {
                ...
                Passed = (Convert.ToDateTime(ruleData.TreatmentToDate).Date >= Convert.ToDateTime(ruleData.EventDate).Date),
        ...
        {
            if (!result.Passed)
                result.MessageList.Add(Constants.TreatmentToDateEventDateValidation);
        }
    }
}
