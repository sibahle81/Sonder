...
namespace RMA.Service.MediCare.RuleTasks.MedicalInvoiceRules.PreAuthToInvoiceAutoMap
{
    public class PreAuthExistRule : IRule
    {
        public const string RuleName = Constants.PreAuthToInvoiceAutoMapRuleName;
        ...
        private RuleResult CreateResult(RuleData ruleData)
        {
            ...
            {
                RuleName = Constants.PreAuthToInvoiceAutoMapRuleName,
                Passed = ruleData.PreAuthCount > 0,
        ...
        {
            if (!result.Passed)
                result.MessageList.Add(Constants.PreAuthExistValidation);
        }
    }
}
