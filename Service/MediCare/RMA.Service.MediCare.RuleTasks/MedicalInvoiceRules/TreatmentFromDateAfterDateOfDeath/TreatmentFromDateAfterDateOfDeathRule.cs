...
{
    public class TreatmentFromDateAfterDateOfDeathRule : IRule
    {
        public const string RuleName = Constants.MedicalInvoiceDateOfDeathRuleName;
        public string Name { get; } = RuleName;
        public string Code { get; } = "MI02";
        ...
        private RuleResult CreateResult(RuleData ruleData)
        {
            var result = new RuleResult
            {
                ...
                Passed = (Convert.ToDateTime(ruleData.TreatmentFromDate).Date <= Convert.ToDateTime(ruleData.DateOfDeath).Date),
        ...
        {
            if (!result.Passed)
                result.MessageList.Add(Constants.TreatmentFromDateDateOfDeathValidation);
        }
    }
}
