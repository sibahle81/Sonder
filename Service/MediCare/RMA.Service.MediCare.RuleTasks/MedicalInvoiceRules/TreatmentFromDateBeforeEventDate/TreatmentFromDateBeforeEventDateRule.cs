...
{
    public class TreatmentFromDateBeforeEventDateRule : IRule
    {
        public const string RuleName = Constants.MedicalInvoiceEventDateRuleName;
        public string Name { get; } = RuleName;
        public string Code { get; } = "MI04";
        ...
        private RuleResult CreateResult(RuleData ruleData)
        {
            ...
            {
                RuleName = Constants.MedicalInvoiceEventDateRuleName,
                Passed = (Convert.ToDateTime(ruleData.TreatmentFromDate).Date >= Convert.ToDateTime(ruleData.EventDate).Date),
        ...
        {
            if (!result.Passed)
                result.MessageList.Add(Constants.TreatmentFromDateEventDateValidation);
        }
    }
}
