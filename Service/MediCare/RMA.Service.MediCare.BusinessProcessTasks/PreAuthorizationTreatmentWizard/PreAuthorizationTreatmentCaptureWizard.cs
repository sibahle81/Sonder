...
{
    public class PreAuthorizationTreatmentCaptureWizard : IWizardProcess
    {
        ...
        public PreAuthorizationTreatmentCaptureWizard(
        ...
        public async Task<int> StartWizard(IWizardContext context)
        {
            ...
            var label = "Treatment Pre-Authorization Form ";
        ...
        public async Task SubmitWizard(IWizardContext context)
        {
            ...
            wizard.LinkedItemId = preAuthId;
            await _wizardService.UpdateWizard(wizard);
            string docKeyName = "MedicalTreatmentPreauthDocumentType";
            //after generating ID update document set with new preAuthId to link Documents to preAuth
            await _documentIndexService.UpdateDocumentKeys(DocumentSystemNameEnum.MediCareManager, docKeyName, wizard.Id.ToString(), docKeyName, preAuthId.ToString());
            ...
            foreach (var recipient in recipients)
            {
                var userReminder = new UserReminder
                {
                    ...
                    Text = $"New Treatment Auth submitted by: {RmaIdentity.UsernameOrBlank}",
                    AlertDateTime = DateTimeHelper.SaNow,
                    CreatedBy = RmaIdentity.UsernameOrBlank,
                    LinkUrl = $"/medicare/treatment-preauth-details/{preAuthId}"
                };
