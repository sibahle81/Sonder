...
{
    public class PreAuthExternalSourceReviewWizard : IWizardProcess
    {
        ...
        public PreAuthExternalSourceReviewWizard(
        ...
        public async Task OnSaveStep(IWizardContext context)
        {
            ...
            try
            {
                var wizard = context.Deserialize<Wizard>(context.Data);
                var stepData = context.Deserialize<ArrayList>(wizard.Data);
                var progressMedicalReport = context.Deserialize<PreAuthorisationForm>(stepData[0].ToString());
            }
            catch (Exception ex)
            {
                ex.LogException();
