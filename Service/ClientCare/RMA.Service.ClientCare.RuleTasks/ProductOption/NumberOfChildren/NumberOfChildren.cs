...
namespace RMA.Service.ClientCare.RuleTasks.ProductOption.NumberOfChildren
{
    public class NumberOfChildren : RuleBase, IRule
    {
        public const string RuleName = "Number Of Children";

        public override string Name { get; } = RuleName;
        public string Code { get; } = "POPT08";
        ...
        RuleResult IRule.Execute(IRuleContext context)
        {
            ...
            if (rules?.Count > 0)
            {
                ...
                {
                    var members = data.Children.Count(m => !m.IsDeleted);
                    if (members > rule.fieldValue)
                    {
                        return GetRuleResult(false, $"Maximum number of {rule.fieldValue} children exceeded.");
                    }
                }
            }
            else
