...
namespace RMA.Service.ClientCare.BusinessProcessTasks.RMLMutualAssuranceQuotationWizard
{
    public class RMLMutualAssuranceQuotationWizard : IWizardProcess
    {
        ...
        public RMLMutualAssuranceQuotationWizard(
        ...
        public async Task SubmitWizard(IWizardContext context)
        {
            ...
            quote.QuoteStatus = QuoteStatusEnum.Quoted;

            await _quoteService.UpdateQuote(quote);

            _ = Task.Run(() => EmailQuote(quote));
        }

        private async Task EmailQuote(QuoteV2 quotation)
        {
            try
            {
                var lead = await _leadService.GetLead(quotation.LeadId);
                ...
                {
                    var productCategoryType = await _productOptionService.GetProductCategoryType(quotation.QuoteDetailsV2[0].ProductOptionId);
                    await _leadCommunicationService.SendRMLAssuranceQuoteEmail(quotation, emailAddresses, parameters, productCategoryType);
                }
            }
            catch (Exception ex)
            {
