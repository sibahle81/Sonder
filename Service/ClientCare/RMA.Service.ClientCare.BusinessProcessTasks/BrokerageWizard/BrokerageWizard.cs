...
{
    public class ManageBrokerageWizard : IWizardProcess
    {
        private readonly IBrokerageService _brokerageService;
        private readonly IDocumentGeneratorService _documentGeneratorService;
        private readonly ISerializerService _serializer;

        public ManageBrokerageWizard(IBrokerageService brokerageService, IDocumentGeneratorService documentGeneratorService, ISerializerService serializerService)
        {
        ...
        public async Task<int> StartWizard(IWizardContext context)
        {
            ...
            brokerage.BrokerageType = BrokerageTypeEnum.Brokerage;
        ...
        private async Task<int> AddWizard(IWizardContext context, ArrayList stepData)
        {
            ...
            var brokerageTypeName = GetBrokerageTypeName(brokerage.BrokerageType);

            var label = !isNew
                ? $"Edit {brokerageTypeName}: {brokerage.Code}"
                : $"New {brokerageTypeName}: {brokerage.Code}";
        ...
        private string GetBrokerageTypeName(BrokerageTypeEnum brokerageType)
        {
            switch (brokerageType)
            {
                case BrokerageTypeEnum.Brokerage:
                    return "Brokerage";
                case BrokerageTypeEnum.BinderPartner:
                    return "Binder Partner";
                case BrokerageTypeEnum.BinderPartnerAndBrokerage:
                    return "Binder Partner and Brokerage";
                default:
                    throw new ArgumentOutOfRangeException(nameof(brokerageType), brokerageType, null);
            }
        }
    }
}
