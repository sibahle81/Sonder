...
{
    public class QlinkTransactionFalsePositiveReportMessageListener : ServiceBusSubscriptionStatelessService<QlinkTransactionResponse>, IQlinkTransactionFalsePositiveReportMessageListener
    {
        private readonly IUserService _userService;
        private readonly IQLinkService _qLinkService;
        public QlinkTransactionFalsePositiveReportMessageListener(
        ...
        private static string GetConnectionString(IConfigurationService configurationService)
        {
            ...
            {
                return GetSetting(configurationService, SystemSettings.QlinkFalsePositiveSubscriptionServiceBusReceiveConnectionString);
            }
            else
            {
                return string.Empty;
        ...
        private static string GetSubscription(IConfigurationService configurationService)
        {
            if (configurationService != null)
            {
                return GetSetting(configurationService, SystemSettings.QlinkFalsePositiveSubscriptionServiceBusReceiveSubscription);
            }
            else
            {
                return string.Empty;
        ...
        private static string GetTopic(IConfigurationService configurationService)
        {
            if (configurationService != null)
            {
                return GetSetting(configurationService, SystemSettings.QlinkFalsePositiveSubscriptionServiceBusReceiveTopic);
            }
            else
            {
                return string.Empty;
        ...
        public override async Task ReceiveMessageAsync(QlinkTransactionResponse qlinkResponseMessage, CancellationToken cancellationToken)
        {
            _ = await _qLinkService.ProcessFalsePositiveQlinkTransactionMessage(qlinkResponseMessage);
        }
