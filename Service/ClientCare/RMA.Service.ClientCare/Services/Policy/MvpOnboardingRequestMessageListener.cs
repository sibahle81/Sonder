...
{
    public class MvpOnboardingRequestMessageListener : ServiceBusSubscriptionStatelessService<PolicyRequestReferenceMessage>, IMvpOnboardingRequestMessageListener
    {
        private readonly IUserService _userService;
        private readonly IMyValuePlusService _myValuePlusService;
        public MvpOnboardingRequestMessageListener(
        ...
        private static string GetConnectionString(IConfigurationService configurationService)
        {
            ...
            {
                return GetSetting(configurationService, SystemSettings.MvpSubscriptionServiceBusReceiveConnectionString);
            }
            else
            {
                return string.Empty;
        ...
        private static string GetSubscription(IConfigurationService configurationService)
        {
            if (configurationService != null)
            {
                return GetSetting(configurationService, SystemSettings.MvpSubscriptionServiceBusReceiveSubscription);
            }
            else
            {
                return string.Empty;
        ...
        private static string GetTopic(IConfigurationService configurationService)
        {
            if (configurationService != null)
            {
                return GetSetting(configurationService, SystemSettings.MvpSubscriptionServiceBusReceiveTopic);
            }
            else
            {
                return string.Empty;
        ...
        {
            _ = await _myValuePlusService.ProcessPolicyRequestReferenceMessageAsync(policyRequestReferenceMessage);
        }
