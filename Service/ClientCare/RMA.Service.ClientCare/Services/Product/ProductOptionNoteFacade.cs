...
{
    public class ProductOptionNoteFacade : RemotingStatelessService, IProductOptionNoteService
    {
        private readonly IDbContextScopeFactory _dbContextScopeFactory;
        private readonly IRepository<product_ProductOptionNote> _noteRepository;
        private readonly IConfigurationService _configurationService;
        private const string ClientModulePermissionsFFL = "ClientModulePermissions";
        public ProductOptionNoteFacade(StatelessServiceContext context,
            IDbContextScopeFactory dbContextScopeFactory,
            IRepository<product_ProductOptionNote> noteRepository,
        ...
        public async Task<int> AddNote(Note noteModel)
        {
            ...
            {
                var entity = Mapper.Map<product_ProductOptionNote>(noteModel);
        ...
        public async Task<List<Note>> GetNotes(int productOptionId)
        {
            if (await _configurationService.IsFeatureFlagSettingEnabled(ClientModulePermissionsFFL) && RmaIdentity.UserId > 0)
                RmaIdentity.DemandPermission(Permissions.ViewProduct);
            ...
            using (_dbContextScopeFactory.CreateReadOnly())
            {
                var notes = await _noteRepository
                    .Where(note => note.ProductOptionId == productOptionId)
                    .ProjectTo<Note>()
                    .ToListAsync();
