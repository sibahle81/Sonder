...
import { AgeAnalysisDatasource } from './age-analysis.datasource';
...
@Component({
  ...
  styleUrls: ['./age-analysis.component.css'],
  providers: [
  ...
  getProducts() {
    ...
    this.productService.getProducts().subscribe(results => {
      ...
      this.products.forEach(s => {
        ...
        lookup.id = s.id;
        lookup.name = s.name;
        this.productLookup.push(lookup);
      });
      this.productLookup.unshift({ id: 0, name: 'All' } as Lookup);
      this.loadingProducts = false;
    });
  };

  getDisplayColumns(): string[] {
    const columns: string[] = ['selected','controlNumber', 'controlName', 'productName', 'productCoverType', 'accountNumber', 'clientName', 'industry', 'interestIndicator', 'status'];
