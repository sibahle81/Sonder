...
export class UploadedFileDetailsDatasource extends Datasource {
    ...
    return merge(...displayDataChanges).pipe(
      map(() => {
        this.filteredData = this.data.slice().filter((item: BulkManualAllocation) => {
          const searchStr = item != null && item.userReference2 != null ? (item.userReference2).toLowerCase() + (item.allocateTo).toString().toLowerCase() : '';
