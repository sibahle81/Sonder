...
export class DebtorPoliciesComponent implements OnInit, OnChanges, AfterViewInit {
  selectedPolicyIds: number[] = [];
  selectedPolicies: PolicyProductCategory[] = [];
  displayedColumns = ['product', 'actions'];
  ...
  displayedChildTableColumns = ['code', 'product', 'policyNumber', 'productBalance', 'compliance', 'actions'];
  isLoadingPolicies$ = new BehaviorSubject(false);
  isAllPoliciesSelected$ = new BehaviorSubject(false);
  isMemberDeclarationCompliant = new BehaviorSubject(false);
  isMemberDBillingCompliant = new BehaviorSubject(false);
  compliant = 'Compliant';
  nonCompliant = 'Non Compliant';
  panelOpenStatePolicies$ = new BehaviorSubject(true);
  complianceCheckDone$ = new BehaviorSubject(false);
  constructor(private roleplayerPolicyService: RolePlayerPolicyService, private productOptionService: ProductOptionService, private readonly billingService: BillingService) { }
  ...
  @Input() checkMemberCompliance: boolean = false;
    ...
    if (event.checked) {
      if (item.categoryPolicies) {
        ...
        item.categoryPolicies.forEach(element => {
          ...
          element.roleplayerId = this.roleplayerId;
  ...
  setComplianceResult($event: ComplianceResult) {
    const complianceResult = $event;
    this.isMemberDeclarationCompliant.next(complianceResult.isDeclarationCompliant);
    this.isMemberDBillingCompliant.next(complianceResult.isBillingCompliant);
    this.complianceCheckDone$.next(true);
  }

  getDebtorPolicies(isInitialProductsLoad = false) {
    this.isLoadingPolicies$.next(true);
    this.billingService.getPolicyProductCategoriesByRoleplayerId(this.roleplayerId).pipe(tap((data) => {
  ...
  getPolicyProductBalances(isInitialProductsLoad: boolean) {
    ...
    this.billingService.getProductBalancesByPolicyIds(this.roleplayerId, policyIds).pipe(
      map(data => {
        if (data && data.length > 0) {
          data.forEach(c => {
