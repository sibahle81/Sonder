...
@Component({
  selector: 'app-bulk-manual-allocations',
  templateUrl: './bulk-manual-allocations.component.html',
  styleUrls: ['./bulk-manual-allocations.component.css']
})
export class BulkManualAllocationsComponent implements OnInit {
    ...
    for (const file of files) {
      ...
      reader.onload = (event: Event) => {
        ...
        if (index >= 0) {
          ...
          this.collectionsService.bulkManualAllocations(content, file.name).subscribe(
            result => {
              if (result === 1) {
                this.isBigFile = true;
              } else if (result === 2) {
              ...
              this.toastr.successToastr(`allocations uploaded from ${file.name}`);
            ...
            (errorResponse: HttpErrorResponse) => {
              ...
              if (errorMessage.substr(0, 10) === 'Validation') {
                ...
                this.alertService.error('Import validations failed', 'Bulk Manual Allocations Error');
                this.downloadReport(fileIdentifier);
              } else {
                this.alertService.parseError(errorResponse, 'Bulk Manual Allocations Error');
              }
              this.uploadControlComponent.isUploading = false;
            }
          );
