...
import { UploadedPaymentFileDetailsDatasource } from './uploaded-payment-file-details.datasourse';

@Component({
  selector: 'app-uploaded-payment-file-details',
  templateUrl: './uploaded-payment-file-details.component.html',
  styleUrls: ['./uploaded-payment-file-details.component.css']
})
export class UploadedPaymentFileDetailsComponent implements OnInit {
  ...
  displayedColumns: string[] = ['userReference', 'userReference2', 'allocateTo', 'amount', 'status', 'error', 'editActions', 'deleteActions'];
  currentQuery = '';
  ...
  constructor(private readonly collectionService: CollectionsService,
    ...
    public datasource: UploadedPaymentFileDetailsDatasource) {
  ...
  loadData() {
    ...
    this.collectionService.getBulkPaymentFileDetails(this.fileId).subscribe(results => {
  ...
  editChecked(event: any, item: BulkManualAllocation) {
    if (event.checked) {
  ...
  deleteChecked(event: any, item: BulkManualAllocation) {
    if (event.checked) {
  ...
  valueInput(row: BulkManualAllocation) {
  ...
  back() {
    this.router.navigate(['fincare/billing-manager/bulk-payment-files/']);
  }
