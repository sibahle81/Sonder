...
export class BankStatementReportComponent implements OnInit {
  ...
  displayedColumns = ['expand', 'bankAccountNumber', 'statementReference', 'userReference', 'userReference2', 'transactionDate', 'amount'];
...
startDateChange() {
  ...
  exportToCSV(): void {
    ...
    this.invoiceService.GetUnallocatedPayments(this.dateFilter, this.startDate, this.endDate, this.searchQuery, bankAccNumber).subscribe(payments => {
      ...
      this.toastr.successToastr('Bank statement exported successfully');
  });
}
