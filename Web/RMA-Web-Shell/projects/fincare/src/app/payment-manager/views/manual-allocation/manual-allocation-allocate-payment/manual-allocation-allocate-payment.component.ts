...
export class ManualAllocationAllocatePaymentComponent implements OnInit {
  ...
  ngOnInit() {
    this.activatedRoute.params.subscribe((params: any) => {
      ...
      if (this.hasPermission || this.hasclaimRecoveryAllocationPermission) {
        this.createForm();
        this.allocationType = params.allocationType;
  ...
  submit() {
    ...
    if (this.allocatableAmount > 0 && this.showBalanceRemainingDialog) {
      ...
      if (this.accountAllocation) {
        ...
        manualPaymentAllocation.unallocatedPaymentId = this.allocationType === 'INV' || this.allocatingOutgoingPayment ? this.paymentId : 0;
        manualPaymentAllocation.unallocatedTransactionId = this.allocationType === 'DBT' ? this.paymentId : 0;
        ...
        manualPaymentAllocation.allocationType = this.actualAllocationType !== '' ? this.actualAllocationType : this.allocationType;
  ...
  invoiceSelected(invoiceId: number, partialAmount: number) {
    ...
    manualPaymentAllocation.unallocatedPaymentId = (this.allocationType === 'INV' || selectedInvoice.isClaimRecoveryInvoice) ? this.paymentId : 0;
    manualPaymentAllocation.unallocatedTransactionId = this.allocationType === 'DBT' ? this.paymentId : 0;
    manualPaymentAllocation.allocatedAmount = selectedInvoice.amountAllocated;
    manualPaymentAllocation.allocationType = this.allocationType;
  ...
  onAccountSelected($event: AccountSearchResult) {
    ...
    if (this.allocationType !== 'DBT') {
      ...
      this.roleplayerService.GetDebtorIndustryClassBankAccountNumber(this.selectedDebtor.finPayeNumber).subscribe(result => {
        const sourceAccNum = this.selectedPayment.bankAccountNumber.replace(/^0+/, '');
        if (result != sourceAccNum) { // LEAVE SINGLE EQUALS
          const isCrossAllocation = !((sourceAccNum == this.individualClassAccNum && result == this.otherClassAccNum) ||
            (sourceAccNum == this.otherClassAccNum && result == this.individualClassAccNum)); // LEAVE DOUBLE EQUALS
  ...
  toggle($event) {
    ...
    this.invoiceAllocation = $event === 'INV';
    this.accountAllocation = $event === 'DBT';
    this.claimRecoveryAllocation = $event === 'REC';
    this.debitTransactionAllocation = $event === 'TRA';
  ...
  debitTransactionSelected(transactionId: number, partialAmount: number) {
    ...
    manualPaymentAllocation.unallocatedPaymentId = this.allocationType === 'DBT' ? 0 : this.paymentId;
    manualPaymentAllocation.unallocatedTransactionId = this.allocationType === 'DBT' ? this.paymentId : 0;
    manualPaymentAllocation.allocatedAmount = selectedTransaction.amountAllocated;
    manualPaymentAllocation.allocationType = 'TRA';
    manualPaymentAllocation.rolePlayerId = this.roleplayerId > 0 ? this.roleplayerId : this.selectedDebtor.rolePlayerId;
