...
@Component({
  selector: 'commission-work-pool-filter',
  templateUrl: './commission-work-pool-filter.component.html',
  styleUrls: ['./commission-work-pool-filter.component.css']
})
export class CommissionWorkPoolFilterComponent extends UnSubscribe implements OnChanges {
  ...
  selectedWorkPoolType = WorkPoolEnum.CommissionPool;
  ...
  getWorkPoolsForUser() {
    this.workPools.forEach(workPool => {
      ...
      if (this.userHasPermission(permissionName)) {
        if (+WorkPoolEnum.CommissionPool == +WorkPoolEnum[workPool]) {
  ...
  setDefaultPool(): WorkPoolEnum {
    ...
    this.selectedWorkPoolType = workPoolType;
    return workPoolType;
  }
