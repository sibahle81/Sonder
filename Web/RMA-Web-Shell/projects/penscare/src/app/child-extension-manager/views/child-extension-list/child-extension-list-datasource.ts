...
@Injectable()
export class ChildToExtensionListDataSource extends PagedDataSource<ChildToAdultPensionLedger> {
  isLoading: boolean;
  isError: boolean;
  filteredData: any[] = [];
  ...
  getData(pageNumber: number = 1, pageSize: number = 5, orderBy: string = 'Name', sortDirection: string = 'asc', query: string = '') {
      ...
      this.pensionLedgerService.searchChildExtensionListWithPagingPagedRequestResult(query, p).pipe(
        catchError(() => of([])),
