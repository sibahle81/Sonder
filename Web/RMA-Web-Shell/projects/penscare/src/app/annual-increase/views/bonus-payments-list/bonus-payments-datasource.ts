...
@Injectable()
export class BonusPaymentsDataSource extends PagedDataSource<PensIncreaseResponse> {
  isLoading: boolean;
  isError: boolean;
  filteredData: any[] = [];
  ...
  getData(pageNumber: number = 1, pageSize: number = 5, orderBy: string = 'CreatedDate', sortDirection: string = 'desc', query: string = 'bonus') {
      ...
      this.annualIncreaseService.getIncreases(sortDirection, query, p).pipe(
        ...
        if(result['data']) {
          result['data'] = result['data'].map((ele) => {
            ele["legislativeValue"] = this.legislativeVal[ele["legislativeValue"]];
