...
@Injectable()
export class MonthlyPensionLedgerListDataSource extends PagedDataSource<MonthlyPensionLedger> {
  isLoading: boolean;
  isError: boolean;
  filteredData: any[] = [];
  ...
  getData(pageNumber: number = 1, pageSize: number = 5, orderBy: string = 'Name', sortDirection: string = 'asc', query: string = '', monthlyPensionId?: number) {
      ...
      this.penscareMonthEndService.getMonthlyPensionLedgersBySearchCriteria(query, p, monthlyPensionId).pipe(
        catchError(() => of([])),
          finalize(() => this.loadingSubject.next(false))
      ).subscribe(result => {
        this.isLoading = false;
        this.data = result as PagedRequestResult<MonthlyPensionLedger>;
