...
import { MedicalFormService } from 'projects/marketingcare/src/app/medical-form-manager/services/medicalform.service';
...
export class WorkItemsListComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    private readonly marketingCareService: MarketingCareService,
  ...
  ngOnInit() {
    this.dataSource = new WorkItemsDataSource(this.marketingCareService);
  ...
  getActiveWorkItems(): void {
    ...
    this.marketingCareService.getFilteredWorkItems(this.workItemTypeFilter, this.workItemCreator).subscribe(data => {
  ...
  routeToMedicalFormView(workItemId: number): void  {
    this.medicalFormService.getMedicalReportFormByWorkItemId(workItemId).subscribe(medicalReportForm => {
      this.router.navigate(['/marketingcare/medical-form-viewer', medicalReportForm.medicalReportFormId]);
    });
  }
}
