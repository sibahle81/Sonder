...
export class CaptureEarnings extends WizardDetailBaseComponent<PersonEventModel> implements OnInit {
  ...
  isOverrideRequired() {
    ...
    if (this.model.earnings?.length == 1) {
      ...
      if (index > -1) {
        ...
        this.earningType = this.model.earnings[0].earningsType;
      }
    }
    ...
    if (isEdit) {
      ...
      this.earningType = this.model.earnings?.some(s => s.earningsType == EarningsTypeEnum.Accident && !s.isEstimated && s.isVerified) ? EarningsTypeEnum.Current : EarningsTypeEnum.Accident;
    }
