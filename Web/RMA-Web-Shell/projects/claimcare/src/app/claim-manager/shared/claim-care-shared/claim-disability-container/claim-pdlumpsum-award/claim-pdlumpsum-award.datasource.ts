...
export class PdAwardDataSource extends PagedDataSource<PdAward> {
  ...
  getData(pageNumber: number = 1, pageSize: number = 5, orderBy: string = 'createdDate', sortDirection: string = 'desc', query: string = '') {
    ...
    this.claimInvoiceService.GetPagedPdLumpSumAwards(pageNumber, pageSize, orderBy, sortDirection, query, this.personEventId).pipe(
      catchError(() => of([])),
      finalize(() => this.loadingSubject.next(false))
    ).subscribe(result => {
      this.data = result as PagedRequestResult<PdAward>;
