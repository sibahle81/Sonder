...
export class ClaimPagedTravelAuthorisationDataSource extends PagedDataSource<TravelAuthorisation> {
  ...
  getData(pageNumber: number = 1, pageSize: number = 5, orderBy: string = 'createdDate', sortDirection: string = 'desc', query: string = '') {
    ...
    this.claimInvoiceService.getPagedClaimTravelAuthorisation(pageNumber, pageSize, orderBy, sortDirection, query, this.personEventId).pipe(
      catchError(() => of([])),
