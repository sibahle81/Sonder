...
export class CadPoolDataSource extends PagedDataSource<CadPool> {
  ...
  getData(pageNumber: number = 1, pageSize: number = 5, orderBy: string = 'DateCreated', sortDirection: string = 'asc', query: string = '') {
    ...
    this.claimCareService.getNonStpCoidPersonEvents(pageNumber, pageSize, orderBy, sortDirection, query).pipe(
      catchError(() => of([])),
