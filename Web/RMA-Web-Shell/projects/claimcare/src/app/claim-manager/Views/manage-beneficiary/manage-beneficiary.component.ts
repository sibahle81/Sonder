...
import { MaritalStatusEnum } from '../../../../../../shared-models-lib/src/lib/enums/marital-status-enum';
...
export class ManageBeneficiaryComponent implements OnInit, OnChanges, AfterViewInit {
  form: FormGroup;
  titles: Lookup[] = [];
  genders: Lookup[] = [];
  ...
  constructor(public dialogRef: MatDialogRef<ManageBeneficiaryComponent>,
  ...
  ngOnInit(): void {
    ...
    this.isSpouse = this.data.isSpouse;
  ...
  patchForm() {
    if (this.beneficiary) {
      ...
      this.form.patchValue({
        ...
        language: this.beneficiary.person.language ? this.beneficiary.person.language : null,
        ...
        beneficiaryType: this.beneficiary.fromRolePlayers.length > 0 ? this.beneficiary.fromRolePlayers[0].rolePlayerTypeId : null,
      });
  ...
  employeeCheck(rolePlayerId: number) {
    if (rolePlayerId === this.selectedPersonEvent.insuredLifeId) {
  ...
  getBeneficiaryName(filteredOption: any) {
    ...
    const beneficiaryType = this.beneficiaryTypes.find(option => option.id === filteredOption || option.name === filteredOption)?.name;
    if (filteredOption === BeneficiaryTypeEnum.Spouse) {
      if ((this.form.get('marriageType').value === MarriageTypeEnum.CivilMarriage || this.form.get('marriageType').value === MarriageTypeEnum.CivilUnion) && this.isSpouse) {
        this.isSpouseError = true;
      } else {
        this.isSpouseError = false;
      }
    } else {
      this.isSpouseError = false;
    }
  ...
  marriageTypeChanged(value: any) {
    if (value.value === MarriageTypeEnum.CivilMarriage || value.value === MarriageTypeEnum.CivilUnion) {
      if (this.form.get('marriageType').value != null && this.form.get('marriageType').value === MaritalStatusEnum.Married) {
        if (this.form.get('beneficiaryType').value != null && this.form.get('beneficiaryType').value === BeneficiaryTypeEnum.Spouse && this.isSpouse) {
          this.isSpouseError = true;
        } else {
          this.isSpouseError = false;
        }
  ...
  save(): void {
    ...
    if (this.beneficiary) {
      this.rolePlayerService.addBeneficiary(this.beneficiary).subscribe(result => {
        if (result) {
          this.beneficiary.rolePlayerId = result;
          const data = {
  ...
  readForm(): RolePlayer {
    ...
    rolePlayer.fromRolePlayers = new Array<RolePlayerRelation>();
    ...
    rolePlayer.rolePlayerTypeId = formDetails.beneficiaryType; // Mapped to the RolePlayerRelation Table
