...
import { InvestigationPoolDataSource } from './investigation-pool.datasource';
...
@Component({
  selector: 'app-investigation-pool',
  templateUrl: './investigation-pool.component.html',
  styleUrls: ['./investigation-pool.component.css']
})
export class InvestigationPoolComponent implements OnInit, AfterViewInit {
  ...
  dataSource: InvestigationPoolDataSource;
  ...
  placeHolder = 'Search by Claim Number, Event Number, Employee Name or Identification Number';
  ...
  displayedColumns: string[] = ['select', 'assignedTo', 'personEventCreatedBy', 'description', 'dateSent', 'dateDue', 'priority', 'eventNumber', 'personEventId', 'lifeAssured',
    'identificationNumber', 'lastModifiedBy', 'application', 'actions'];


  ngOnInit(): void {
    this.isLoading = true;
    this.dataSource = new InvestigationPoolDataSource(this.claimCareService);
  ...
  sortData(dataSource: any): CadPool[] {
    ...
    return dataSource.data.sort((a, b) => {
      ...
      switch (this.sort.active) {
        ...
        case 'lastModified': [propertyA, propertyB] = [a.lastModifiedBy, b.lastModifiedBy]; break;
  ...
  allocateUserToClaim(allocateUserToClaim, item: any): void {
    let valueExist = false;
    if (item !== null) {
