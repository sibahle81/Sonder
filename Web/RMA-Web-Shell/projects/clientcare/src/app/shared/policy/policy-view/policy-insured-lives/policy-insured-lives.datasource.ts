...
export class PolicyInsuredLivesDataSource extends PagedDataSource<PolicyGroupMember> {
  ...
  getData(pageNumber: number = 1, pageSize: number = 5, orderBy: string = 'rolePlayerId', sortDirection: string = 'desc', query: string = '') {
    ...
    this.policyService.getPagedPolicyInsuredLives(pageNumber, pageSize, orderBy, sortDirection, query).pipe(
      catchError(() => of([])),
