...
//import { StageGroupRiskMember } from '../../shared/entities/stage-group-risk-member';
...
import { VopdManualVerificationDialogComponent } from '../../../shared/vopd/vopd-manual-verification-dialog/vopd-manual-verification-dialog.component';
import { MatDialog } from '@angular/material/dialog';
import { userUtility } from 'projects/shared-utilities-lib/src/lib/user-utility/user-utility';

@Component({
  selector: 'app-group-risk-members',
  templateUrl: './group-risk-members.component.html',
  styleUrls: ['./group-risk-members.component.css'],
  providers: [
    { provide: DateAdapter, useClass: MatDatePickerDateFormat },
    { provide: MAT_DATE_FORMATS, useValue: DatePickerDateFormat }
  ]
})
export class GroupRiskMembersComponent extends WizardDetailBaseComponent<GroupRisk> {

  isLoading$: BehaviorSubject<boolean> = new BehaviorSubject(false);
  members: MemberVopdStatus[] = [];
  dataSource = new MatTableDataSource<MemberVopdStatus>();
  //To implement VOPD in the future sprint
 // displayedColumns = ['employeeIndustryNumber', 'employeeNumber','firstName' ,'surname','idOrPassport', 'dateOfBirth', 'employeeStartDate','monthlyRiskSalary', 'vopdProcessStatus', 'vopdStatus', 'dateVerified', 'actions'  ];
// displayedColumns = ['employeeIndustryNumber', 'employeeNumber','firstName' ,'surname','idOrPassport', 'dateOfBirth', 'employeeStartDate','monthlyRiskSalary', 'note' ];
displayedColumns = ['memberType', 'memberName', 'idNumber', 'dateOfBirth', 'age', 'joinDate', 'vopdProcessStatus', 'vopdStatus', 'dateVerified', 'actions'];
 menus: { title: string, action: string, disable: boolean }[];
  ...
  populateForm(): void {
    ...
    this.groupRiskService.getGroupRiskVopdStatus(this.model.fileIdentifier).subscribe({
  ...
  openVopdEditDialog(item: MemberVopdStatus): void {
    ...
    const dialogRef = this.dialog.open(VopdManualVerificationDialogComponent, { width: '800px', height: '800px', data: { memberName: item.memberName, idNumber: item.idNumber, canEditVopd, isAlive, dateOfDeath, fileIdentifier } });
