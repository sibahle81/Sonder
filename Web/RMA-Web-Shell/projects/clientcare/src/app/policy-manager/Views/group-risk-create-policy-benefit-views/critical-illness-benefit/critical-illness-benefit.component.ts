...
@Component({
  selector: 'create-policy-critilcalIllness-benefit',
  templateUrl: './critical-illness-benefit.component.html',
  styleUrls: ['./critical-illness-benefit.component.css']
})
export class CriticalIllnessBenefitComponent extends WizardDetailBaseComponent<GroupRiskPolicyCaseModel> {
  @Output() survivalPeriodChanged = new EventEmitter<number>();

  benefitBasisOptions: OptionItemValueLookup[];
  continuedCoverOptions: OptionItemValueLookup[];
  topUpBenefitOptions: OptionItemValueLookup[];
  restoreDeathOptions: OptionItemValueLookup[];
  survivalPeriodOptions: OptionItemValueLookup[];
  ...
  createForm() {
    this.form = this.formBuilder.group({
      ...
      topUpBenefit: ["", Validators.required],
      survivalPeriod: ["", Validators.required],
      restoreDeathBenfit: ["", Validators.required],
      maximumBenefitMultiple: [""],
      maximumSumAssured: [""]
    });
  }
  ...
  loadLookups(benefitId: number) {
    ...
    this.groupRiskPolicyCaseService.getOptionItems(GroupRiskOptionTypeLevelEnum.Benefit, "TopUpBenefit", benefitId).subscribe(
      {
        next: result => {
          this.topUpBenefitOptions = result;
        },
        error: err => {
          this.privateAlertService.error(
            "An error occured while trying to fetch options.",
    ...
    this.groupRiskPolicyCaseService.getOptionItems(GroupRiskOptionTypeLevelEnum.Benefit, "SurvivalPeriod", benefitId).subscribe(
      {
        next: result => {
          this.survivalPeriodOptions = result;
        },
        error: err => {
          this.privateAlertService.error(
            "An error occured while trying to fetch options.",
    ...
    this.groupRiskPolicyCaseService.getOptionItems(GroupRiskOptionTypeLevelEnum.Benefit, "RestoreDeathBenefit", benefitId).subscribe(
      {
        next: result => {
          this.restoreDeathOptions = result;
        },
        error: err => {
          this.privateAlertService.error(
            "An error occured while trying to fetch options.",
  ...
  onSurvivalPeriodChanged($event) {
    this.survivalPeriodChanged.emit($event.value);
  }
