...
@Component({
  templateUrl: './upload-myvalueplus.component.html',
  styleUrls: ['./upload-myvalueplus.component.css']
})
export class UploadMyvalueplusComponent implements OnInit {
  ...
  private readonly memberSheetName: string = 'Onboarding';
            ...
            if (jsonContent.length > 4) {
              ...
              this.documentManagementService.UploadDocument(doc).subscribe({
                next: () => {
                  ...
                  const uploadData = { data: fileContent, FileAsBase64 : fileData};
                  var policyNumber = String.isNullOrEmpty(values.policyNumber) ? "none" : values.policyNumber;
                  this.policyService.uploadMyValuePlus(file.name, uploadData, values.policyOption, policyNumber).subscribe({
                    next: (data: string[]) => {
                      if (data.length > 0) {
                        ...
                        this.alertService.success(`${file.name} successfully uploaded`, 'My Value Plus');
                    ...
                    error: (errorResponse: HttpErrorResponse) => {
                      this.alertService.error('File Upload Error', 'My Value Plus Error');
                ...
                error: (response: HttpErrorResponse) => {
                  this.alertService.parseError(response, 'My Value Plus Error');
                  this.uploadControlComponent.isUploading = false;
                  this.disabled = false;
                }
              });
