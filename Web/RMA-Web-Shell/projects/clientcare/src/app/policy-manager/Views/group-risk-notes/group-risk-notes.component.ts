...
@Component({
  selector: 'app-group-risk-notes',
  templateUrl: './group-risk-notes.component.html',
  styleUrls: ['./group-risk-notes.component.css']
})
export class GroupRiskNotesComponent extends WizardDetailBaseComponent<GroupRiskPolicyCaseModel>{
    ...
    DeletedNote(commonNodeId: number){
        if(this.model)
        {
            const noteIndex = this.model.groupRiskNotes.indexOf(commonNodeId);
            if(noteIndex != -1)
            {
                this.model.groupRiskNotes.splice(noteIndex, 1);
            }
        }
    }
