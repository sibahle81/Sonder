...
export class BrokerageNotesComponent extends NotesComponent implements WizardComponentInterface, OnInit {
    ...
    wizardReadFormData(context: WizardContext): Brokerage {
        const brokerage = context.data[0] as Brokerage;
        brokerage.brokerageNotes = this.notes;
        this.brokerage = brokerage;
        return brokerage;
    }

    wizardPopulateForm(context: WizardContext): void {
        this.brokerage = context.data[0] as Brokerage;
        this.notes = [];
        this.mode = 'view';
        ...
        if (this.brokerage !== null && this.brokerage.brokerageNotes != null && this.brokerage.brokerageNotes.length > 0) {
            this.notes = this.brokerage.brokerageNotes;
        }
    ...
    wizardValidateForm(context: WizardContext): ValidationResult {
        ...
        if (!brokerage.brokerageNotes === undefined || brokerage.brokerageNotes.length === 0 || brokerage.brokerageNotes.findIndex(n => n.id === 0) === -1) {
    ...
    readForm(): Note {
        if (this.isWizard) {
            ...
            this.selectedNote.itemType = 'brokerage';
            this.selectedNote.itemId = this.brokerage.id;
