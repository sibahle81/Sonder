...
@Injectable()
export class ProductLastViewedListDataSource extends Datasource {
    ...
    getData() {
        this.isLoading = true;
        this.productService.getLastViewedItems().subscribe(
            data => {
    ...
    connect(): Observable<LastViewedItem[]> {
        ...
        return merge(...displayDataChanges).pipe(map(() => {
            this.filteredData = this.data.slice().filter((item: LastViewedItem) => {
                const searchStr = item.code + '|' + item.name + '|' + item.description;
