...
export class ProductNotesComponent extends NotesComponent implements WizardComponentInterface, OnInit {
    ...
    constructor(
        ...
        this.addPermission = 'Add Product';
        this.editPermission = 'Edit Product';
    }

    wizardReadFormData(context: WizardContext): Product {
        const product = context.data[0] as Product;
        product.productNotes = this.notes;
        this.product = product;
        return product;
    }

    wizardPopulateForm(context: WizardContext): void {
        this.product = context.data[0] as Product;
        this.notes = [];
        this.mode = 'view';
        ...
        if (this.product !== null && this.product.productNotes != null && this.product.productNotes.length > 0) {
            this.notes = this.product.productNotes;
        }
    ...
    wizardValidateForm(context: WizardContext): ValidationResult {
        ...
        if (!product.productNotes || product.productNotes.length === 0 || product.productNotes.findIndex(n => n.id === 0) === -1) {
    ...
    readForm(): Note {
        if (this.isWizard) {
            ...
            this.selectedNote.itemType = 'product';
            this.selectedNote.itemId = this.product.id;
