...
import { BenefitSearchDataSource } from '../../datasources/benefit-search.datasource';
...
@Component({
    ...
    styleUrls: ['./benefit-search.component.css'],
    selector: 'benefit-search'
})
export class BenefitSearchComponent implements OnInit, AfterViewInit {
    form: UntypedFormGroup;
    currentQuery: string;
    displayedColumns = ['name', 'code', 'modifiedBy', 'modifiedDate', 'statusText', 'actions'];
    ...
    ngOnInit(): void {
        this.dataSource = new BenefitSearchDataSource(this.benefitService);
    }
    ...
    onSelect(item: Benefit): void {
        this.router.navigate(['clientcare/product-manager/benefit-details', item.id]);
    }
    ...
    addBenefit(): void {
        const question = `Please note that this will create a new wizard. Are you sure you want to start a new wizard?`;
    ...
    startWizard() {
        ...
        startWizardRequest.type = 'benefit';
        this.wizardService.startWizard(startWizardRequest).subscribe(result => {
            this.alertService.successToastr('Benefit wizard started successfully');
