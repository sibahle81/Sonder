...
import { WorkItemTypeEnum } from 'projects/legalcare/src/app/work-manager/models/enum/work-item-type.enum';
...
export class WorkItemsListComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    this.dataSource = new WorkItemsDataSource(this.legalCareService);
  ...
  getActiveWorkItems(): void {
    ...
    this.legalCareService.getFilteredWorkItems(this.workItemTypeFilter, this.workItemCreator).subscribe(data => {
  ...
  routeToMedicalFormView(workItemId: number): void  {
    this.medicalFormService.getMedicalReportFormByWorkItemId(workItemId).subscribe(medicalReportForm => {
      this.router.navigate(['/legalcare/medical-form-viewer', medicalReportForm.medicalReportFormId]);
    });
  }
}
