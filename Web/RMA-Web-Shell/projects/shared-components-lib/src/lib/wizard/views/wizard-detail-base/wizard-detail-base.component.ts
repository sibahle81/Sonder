...
import { QueryList, ViewChildren, OnInit, Directive } from '@angular/core';
import { AppEventsManager } from 'projects/shared-utilities-lib/src/lib/app-events-manager/app-events-manager';
import { MultiSelectComponent } from 'projects/shared-components-lib/src/lib/multi-select/multi-select.component';
...
export abstract class WizardDetailBaseComponent<TModel> implements WizardComponentInterface, OnInit {
    ...
    if (this.isWizard) {
      if (this._context && this._context.wizard) {
        return this._context.wizard.wizardStatusId === WizardStatus.InProgress
          || this._context.wizard.wizardStatusId === WizardStatus.Disputed;
      }
    }
    return false;
  }
  ...
  wizardValidateForm(context: WizardContext): ValidationResult {
    if (context.name !== 'premium-listing' && context.name !== 'cfp-onboarding'  &&  context.name !== 'grouprisk-onboarding'  && context.name !== 'mvp-onboarding') {
      ...
      Object.keys(this.form.controls).forEach(key => {
        ...
        if (control.status === 'PENDING') {
          ...
          if (control.hasError('required') || control.hasError('min')) {
            ...
            if (control.status === 'INVALID' && control.errors && control.errors.minlength) {
              validationResult.errorMessages.push(`Field "${key}" minimum ${control.errors.minlength.requiredLength} characters required.`);
            } else {
  ...
  getLookupControl(lookupName: string) {
    ...
    const component = this.multiSelectComponentChildren.find((child) => child.lookupName === lookupName || child.ItemList === lookupName);
    return component;
  }
  ...
  protected checkUserAddPermission(): void {
    ...
    if (this.addPermission !== undefined && this.addPermission !== '') {
      this.canAdd = userUtility.hasPermission(this.addPermission);
    }
    if (this.editPermission !== undefined && this.editPermission !== '') {
      this.canEdit = userUtility.hasPermission(this.editPermission);
    }
    if (this.viewPermission !== undefined && this.viewPermission !== '') {
      this.canView = userUtility.hasPermission(this.viewPermission);
    }
  ...
  get inApprovalMode(): boolean {
    return this.context && this.context.wizard && (this.context.wizard.wizardStatusId === WizardStatus.AwaitingApproval || this.context.wizard.wizardStatusId === WizardStatus.Completed);
  }
