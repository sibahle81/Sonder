...
import { UploadService } from './upload-control.service';
...
export class UploadControlComponent implements OnInit {
  ...
  resetUpload(): void {
    ...
    this.uploadFileList = new Array<UploadFile>();
  }
  ...
  readFiles(files: Array<File>) {
    ...
    if (this.validateNumberOfFiles(this.uploadFileList.length + 1)
  ...
  uploadFiles() {
    if (this.uploadFileList.length > 0) {
      ...
      this.uploadService.uploadFile(formData).subscribe(
        data => {
          this.loadingMessage$.next('uploading document...please wait');
          this.isUploading = false;
          this.isLoading$.next(false);
  ...
  validateFileTypes(): boolean {
    const fileTypeArray = this.acceptedTypes ? this.acceptedTypes.split(',').map(i => i.replace('.', '').toLocaleUpperCase()) : null;
  ...
  formatText(text: string): string {
    return text ? text.replace(/((?<=[a-z])[A-Z]|[A-Z](?=[a-z]))/g, ' $1').trim() : '<no data>';
  }
}
