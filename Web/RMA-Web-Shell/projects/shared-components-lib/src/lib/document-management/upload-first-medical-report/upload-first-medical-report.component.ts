...
import { UploadFile } from '../../upload-control/upload-file.class';
...
@Component({
  selector: 'lib-upload-first-medical-report',
  templateUrl: './upload-first-medical-report.component.html',
  styleUrls: ['./upload-first-medical-report.component.css'],
  providers: [
    { provide: DateAdapter, useClass: MatDatePickerDateFormat },
    { provide: MAT_DATE_FORMATS, useValue: DatePickerDateFormat }
  ]
})
export class UploadFirstMedicalReportComponent implements OnInit {
  ...
  public userHealthCareProviders: HealthCareProvider[];
  ...
  uploadedMedicalReport: UploadFile[];
  hasDocuments = false;
  isSaving = false;
  ...
  constructor(
    public dialogRef: MatDialogRef<UploadFirstMedicalReportComponent>,
  ...
  ngOnInit() {
    ...
    this.eventModel = this.data.event;
    ...
    if (this.data.personEvent.firstMedicalReport) {
      this.patchForm(this.data.personEvent.firstMedicalReport);
    }

    if (this.data.personEvent && this.data.personEvent.physicalDamages.length > 0) {
  ...
  createForm() {
    this.form = this.formBuilder.group({
      ...
      dateOfConsultation: new UntypedFormControl('', [Validators.required, DateValidator.checkIfDateLessThan('dateOfConsultation', this.datepipe.transform(this.data.event.eventDate, Constants.dateString))]),
  ...
  readForm(): FirstMedicalReportForm {
    ...
    if (this.form.controls.isUnfitForWork.value && this.form.controls.isUnfitForWork.value === 'Yes') {
      ...
      firstMedicalReport.estimatedDaysOff = this.form.controls.estimatedDaysOff.value ? this.form.controls.estimatedDaysOff.value : null;
    }
  ...
  patchForm(medicalReport: FirstMedicalReportForm) {
    if (medicalReport) {
      ...
      if (this.data.personEvent.firstMedicalReport.medicalReportForm.icd10CodesJson) {
        // tslint:disable-next-line: no-shadowed-variable
        const icd10CodesJson =  JSON.parse(this.data.personEvent.firstMedicalReport.medicalReportForm.icd10CodesJson);
  ...
  getDocumentTypesByDocumentSet(documentSet: DocumentSetEnum) {
    this.documentManagementService.GetDocumentTypesBySetId(documentSet).subscribe(results => {
      this.documentTypes = results.filter(r => r.name === Constants.firstMedicalReport);
      ...
      reader.onload = () => {
        ...
        this.documentManagementService.UploadDocument(document).subscribe(result => {
          if (result.documentExist) {
            ...
            this.alertService.success('First medical report uploaded sucessfully');
            firstMedicalReport.medicalReportForm.documentId = result.id;
