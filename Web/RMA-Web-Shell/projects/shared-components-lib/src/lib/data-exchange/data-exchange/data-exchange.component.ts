...
export class DataExchangeComponent {
  ...
  onFileChange(evt: any) {
    ...
    if (this.files[0].type != 'application/vnd.ms-excel' && this.files[0].type != 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' && this.files[0].type != 'text/csv') {
      this.openMessageDialog('Invalid File Type', 'Only .xlsx, .xls or csv files are supported');
      return;
    }
  ...
  generateTemplate() {
    if (this.expectedColumnHeadings.length > 0) {
      ...
      this.export(template, 'template_data');
    }
  }

  exportInvalid() {
    this.export(this.invalidData, 'invalid_data');
  }
