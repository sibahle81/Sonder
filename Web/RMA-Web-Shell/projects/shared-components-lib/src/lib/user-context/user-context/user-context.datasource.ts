...
export class UserContextDataSource extends PagedDataSource<LinkedUserMember> {
  ...
  getData(pageNumber: number = 1, pageSize: number = 5, orderBy: string = 'rolePlayerId', sortDirection: string = 'desc', query: string = '') {
    ...
    this.memberService.getPagedLinkedUserMembers(pageNumber, pageSize, orderBy, sortDirection, query).pipe(
      catchError(() => of([])),
      finalize(() => this.loadingSubject.next(false))
    ).subscribe(result => {
      this.data = result as PagedRequestResult<LinkedUserMember>;
