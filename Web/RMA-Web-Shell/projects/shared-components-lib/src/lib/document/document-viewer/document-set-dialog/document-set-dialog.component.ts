...
export class DocumentSetDialogComponent implements OnInit {
  ...
  existingDocumentSets: DocumentSetEnum[];
  ...
  constructor(
    ...
    this.existingDocumentSets = data.existingDocumentSets;
  }
  ...
  filterDataSets() {
    this.existingDocumentSets.forEach(existingDoumentSet => {
      const index = this.allDocumentSets.findIndex(s => DocumentSetEnum[DocumentSetEnum[s]] === DocumentSetEnum[existingDoumentSet]);
      if (index !== -1) {
        this.allDocumentSets.splice(index, 1);
      }
    });
  }
