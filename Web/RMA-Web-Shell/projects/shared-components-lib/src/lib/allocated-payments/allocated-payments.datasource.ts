...
import { AllocatedPayment } from 'projects/fincare/src/app/shared/models/allocated-payment';


@Injectable()
export class AllocatedPaymentsDatasource extends Datasource {
  isLoading = true;
  isError = false;
  ...
  getData(data: any): void {
    this.startLoading('Loading allocated payments...');
    ...
    const bankAccNumber = data.bankAccount ? data.bankAccount : '0';
    ...
    this.invoiceService.getAllocatedPayments(startDate, endDate, dateType, bankAccNumber, productId, periodYear, periodMonth).subscribe(payments => {
  ...
  connect(): Observable<AllocatedPayment[]> {
    ...
    return merge(...displayDataChanges).pipe(map(() => {
      this.filteredData = this.data.slice().filter((item: AllocatedPayment) => {
        const searchStr = (item.userReference).toLowerCase() + (item.policyNumber).toLowerCase() +
          (item.debtorName).toLowerCase() +
          (item.invoiceNumber).toLowerCase() +
          (item.debtorNumber).toLowerCase() +
  ...
  getSortedData(data: AllocatedPayment[]): AllocatedPayment[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this.sort.active) {
        ...
        case 'debtorNumber': [propertyA, propertyB] = [a.debtorNumber, b.debtorNumber]; break;
      }
