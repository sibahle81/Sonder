...
export class MyWorkflowsDataSource extends PagedDataSource<Wizard> {
  ...
  getData(pageNumber: number = 1, pageSize: number = 5, orderBy: string = 'createdDate', sortDirection: string = 'desc', query: string = '') {
    ...
    this.wizardService.getPagedUserWizardsByConfigsAndAssignedTo(this.wizardConfigs, pageNumber, pageSize, orderBy, sortDirection, query).pipe(
      catchError(() => of([])),
