...
export class RolePlayerSearchDataSource extends PagedDataSource<RolePlayer> {
  ...
  getData(pageNumber: number = 1, pageSize: number = 5, orderBy: string = 'modifiedDate', sortDirection: string = 'desc', query: string = '') {
    ...
    this.rolePlayerIdentificationTypeId = this.rolePlayerIdentificationTypeId ? this.rolePlayerIdentificationTypeId : 0;

    this.rolaPlayerService.getPagedRolePlayers(this.rolePlayerIdentificationTypeId, pageNumber, pageSize, orderBy, sortDirection, query).pipe(
      catchError(() => of([])),
