...
export class ProstheticPreAuthReviewWizardContext extends WizardContext {
  ...
  constructor(componentFactoryResolver: ComponentFactoryResolver) {
    ...
    this.currentUser = this.authService.getCurrentUser();
        if(this.currentUser?.isInternalUser){    
           this.wizardComponents.push(new WizardComponentStep(0, 'Review Prosthetic Auth', ProstheticPreAuthReviewerComponent));
        }
        else{
          this.wizardComponents.push(new WizardComponentStep(0, 'Holistic View', WizardMediHolisticViewComponent));
          this.wizardComponents.push(new WizardComponentStep(1, 'Review Prosthetic Auth', ProstheticPreAuthReviewerComponent));
         }
  }
