...
@Injectable()
export class PreAuthorisationSearchDataSource extends PagedDataSource<PreAuthorisation> {
    ...
    getData(pageNumber: number, pageSize: number, orderBy: string = 'preAuthId', sortDirection: string = 'asc', query: string) {
        ...
        this.mediCarePreAuthService.searchPreAuthorisation(pageNumber, pageSize, orderBy, sortDirection, query).pipe(
            catchError(() => of([])),
