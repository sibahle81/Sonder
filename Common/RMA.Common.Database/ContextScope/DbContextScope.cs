...
namespace RMA.Common.Database.ContextScope
{
    public class DbContextScope : IDbContextScope
    {
        ...
        {
            if (_disposed) throw new ObjectDisposedException("DbContextScope");
            if (_completed)
            {
                throw new InvalidOperationException(
        ...
        public void RefreshEntitiesInParentScope(IEnumerable entities)
        {
            ...
            {
                if (!(_parentScope._dbContexts.InitializedDbContexts.Values.SingleOrDefault(parentContext =>
                    parentContext.GetType() == contextInCurrentScope.GetType()) is IObjectContextAdapter correspondingParentContext))
                ...
                foreach (var toRefresh in entities)
                {
                        ...
                        out var stateInCurrentScope))
                    {
                        ...
                        if (correspondingParentContext.ObjectContext.ObjectStateManager.TryGetObjectStateEntry(key,
                                out var stateInParentScope) && stateInParentScope.State == EntityState.Unchanged)
                        {
        ...
        public async Task RefreshEntitiesInParentScopeAsync(IEnumerable entities)
        {
            ...
            {
                if (!(_parentScope._dbContexts.InitializedDbContexts.Values.SingleOrDefault(parentContext =>
                    parentContext.GetType() == contextInCurrentScope.GetType()) is IObjectContextAdapter correspondingParentContext)) continue;
                        ...
                        out var stateInCurrentScope))
                    {
                                ...
                                out var stateInParentScope) && stateInParentScope.State == EntityState.Unchanged)
                        {
                            await correspondingParentContext.ObjectContext
                               .RefreshAsync(RefreshMode.StoreWins, stateInParentScope.Entity).ConfigureAwait(false);
                        }
                    }
                }
            }
...
" + Environment.StackTrace;
        ...
        private int CommitInternal()
        {
            ...
            {
                throw DbContextExtensions.HandleDbValidationErrors(errors.EntityValidationErrors);
            }
        }
        ...
        internal static void RemoveAmbientScope()
        {
            ...
            if (CallContext.LogicalGetData(AmbientDbContextScopeKey) is InstanceIdentifier current) DbContextScopeInstances.Remove(current);
        }
