...
{
    [EventSource(Name = "RMA.ServiceFabric")]
    public sealed class ServiceEventSource : EventSource
    {
        ...
        {
            if (serviceContext == null) throw new ArgumentNullException(nameof(serviceContext), "serviceContext is null");
            if (IsEnabled())
            {
                Log.Information(message, args);
                var finalMessage = string.Format(message, args);
 ...
 SizeInBytes(applicationTypeName) };
        ...
        {
            Log.Information("Service host process {hostProcessId} registered service type {serviceType}", hostProcessId,
                serviceType);
        ...
        {
            Log.Information("Service host initialization failed - {exception}", exception);
        ...
        {
            Log.Information("Service request '{requestTypeName}' started", requestTypeName);
        ...
        {
            Log.Information("Service request '{requestTypeName}' finished", requestTypeName, exception);
