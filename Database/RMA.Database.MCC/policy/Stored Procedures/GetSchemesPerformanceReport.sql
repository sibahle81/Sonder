...
-- EXEC [policy].[GetSchemesPerformanceReport] 'July', 2024
-- =============================================
CREATE PROCEDURE [policy].[GetSchemesPerformanceReport]
...
insert into @policy ([Brokerage], [Scheme], [Policies], [Lives], [Premium])
	...
	select [Scheme], 
			...
			[Policies], 
			[Lives], 
			cast([Premium] as money) [Premium]
	from @policy 
	where [Scheme] <> 'Individual' 
