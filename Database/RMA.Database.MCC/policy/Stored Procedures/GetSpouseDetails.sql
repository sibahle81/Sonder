...
CREATE   PROCEDURE [policy].[GetSpouseDetails]
	@PolicyId INT
AS
BEGIN
	select Distinct
	      [SpouseName] = p.FirstName+' '+ p.Surname,
		  [SpouseIdNo] = p.IdNumber,
          [SpouseDOB] = convert(datetime,SWITCHOFFSET(convert(datetimeoffset,p.DateOfBirth),'+02:00')),
          [SpouseBenefitAmount] = benefitRate.BenefitAmount,
		...
		where pol.PolicyId = @PolicyId AND rp.RolePlayerTypeId = 11
END