CREATE PROCEDURE [policy].[NewBusinessCOIDPoliciesCLASSIV]
AS
BEGIN

	IF(OBJECT_ID('tempdb..#TempNewBusiness') Is Not Null)
		BEGIN
			DROP TABLE #TempNewBusiness
		END
	
	DECLARE @MAX INT = 0;

	SELECT number as value, DATENAME(MONTH, '2012-' + CAST(number as varchar(2)) + '-1') Month INTO #TempNewBusiness
	FROM master..spt_values
	WHERE Type = 'P' AND number BETWEEN 1 AND DATEPART(MONTH, GETDATE())
	ORDER BY Number

	SET @MAX = (SELECT COUNT(*) FROM #TempNewBusiness);

	ALTER TABLE #TempNewBusiness
	...
	BEGIN
		   ...
		   FROM [policy].[Policy] PP INNER JOIN 
			...
			AND CC.IndustryClassId = 4 and DATEPART(MONTH, PP.CreatedDate) = @COUNT
			GROUP BY DATENAME(month,PP .CreatedDate),DATEPART(MONTH, PP.CreatedDate)	   

	

			
					UPDATE #TempNewBusiness 
