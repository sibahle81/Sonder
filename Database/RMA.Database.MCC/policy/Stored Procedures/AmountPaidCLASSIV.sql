CREATE PROCEDURE [policy].[AmountPaidCLASSIV]
AS
BEGIN


If(OBJECT_ID('tempdb..#tempAmountPaidCLASSIV') Is Not Null)
Begin
    Drop Table #tempAmountPaidCLASSIV
End

create table #TempAmountPaidCLASSIV
(
...
INSERT INTO #TempAmountPaidCLASSIV(ROEStatus)
 VALUES ('Not Updated')
...
SET @NotUpdatedPaid = (SELECT COUNT(*) 
						   FROM [client].[Declaration] INNER JOIN
							...
							WHERE [billing].[Invoice].InvoiceStatusId = 1 AND [common].[DeclarationType].id <> 2 AND [Company].IndustryClassId = 4)
...
SET @NotUpdatedShortPaid = (SELECT COUNT(*) 
						    FROM [client].[Declaration] INNER JOIN
								...
								[policy].[Policy] ON [policy].[Policy].[PolicyOwnerId] = [client].[RolePlayer].RolePlayerId INNER JOIN
								[billing].[Invoice] ON [billing].[Invoice].[PolicyId] =  [policy].[Policy].PolicyId 
							WHERE [billing].[Invoice].InvoiceStatusId = 4 AND [common].[DeclarationType].id <> 2 AND [Company].IndustryClassId = 4)
...
SELECT * FROM #TempAmountPaidCLASSIV
