CREATE PROCEDURE [policy].[AmountInvoicedCLASSIV]
AS 
BEGIN
		IF(OBJECT_ID('tempdb..#TempAmountInvoicedCLASSIV') Is Not Null)
		BEGIN
			DROP TABLE #TempAmountInvoicedCLASSIV
		END

		CREATE TABLE #TempAmountInvoicedCLASSIV
		(
		...
		INSERT INTO #TempAmountInvoicedCLASSIV(ROEStatus)
			VALUES ('Not Updated')
		...
		SET @NotUpdatedNoOfMembers = ISNULL((SELECT COUNT(*) 
									...
									WHERE [common].[DeclarationType].[id] <> 2 AND [Company].[IndustryClassId] = 4),0)
		...
		UPDATE #TempAmountInvoicedCLASSIV
		...
		SELECT * FROM #TempAmountInvoicedCLASSIV
