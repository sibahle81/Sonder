CREATE PROCEDURE [client].[EstimatesReport]
	@StartDate DATE = NULL, 
	@EndDate DATE = NULL,
...
BEGIN
	WITH latestSubmissionForCoverPeriod AS (
	  ...
	  WHERE [DECLARATION].RolePlayerPolicyDeclarationTypeId = 1 -- estimates
	)
	SELECT
		...
		,[latestSubmissionForCoverPeriod].DeclarationYear AS [Cover Period]
		,[TYPE].[Name] AS [Type Submitted]
		,[latestSubmissionForCoverPeriod].[ModifiedDate] AS [Date Submitted]
