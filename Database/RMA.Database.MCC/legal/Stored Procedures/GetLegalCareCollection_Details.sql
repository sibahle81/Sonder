...
CREATE   Procedure [legal].[getLegalCareCollection_Details]
(
@userId int,
@PageNumber AS INT,
@RowsOfPage AS INT,
...
SET @SelectCount = (select 
	...
	where 
		...
		and [legal].[LegalCareCollection].LegalCareCollectionStatusId = @Status
...
SET @RecordCount = (select 
	...
	select 
		...
		[legal].[LegalCareCollection].LegalCareCollectionStatusId [Status],
	...
	where 
		...
		and [legal].[LegalCareCollection].LegalCareCollectionStatusId = @Status 
		and  ([client].[RolePlayer].DisplayName like ('%'+ @SearchCreatia +'%') )
		order by [client].[RolePlayer].DisplayName asc,  		[legal].[LegalCareCollection].CreatedDate  desc 
		
		OFFSET (@PageNumber+-1)* @RowsOfPage
		ROW FETCH NEXT @RowsOfPage ROWS ONLY
END
--exec [legal].[GetLegalCareCollection_Details] 1009976,1,10,'','','',2  -- Parameter 23 = UserId,  Status = 'Open = 1, Ongoing=2, Closed =3'   