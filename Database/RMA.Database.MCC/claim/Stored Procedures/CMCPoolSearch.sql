...
CREATE PROCEDURE [claim].[CMCPoolSearch] 
(
	@PageNumber		AS INT,
	@RowsOfPage		AS INT,
...
BEGIN
	-- === DEBUG ========================================================= --
		...
		--DECLARE	@SortingCol		AS VARCHAR(100) =	'CreatedDate';
...
BEGIN -- EXECUTE
    ...
    SET @FinalQuery = '
        ...
        FROM (
            SELECT *,
                ROW_NUMBER() OVER (ORDER BY ' + @SortingCol + ' ' + @SortType + ') AS RowNum
            FROM #CMCPool
        ) AS TempTable
        WHERE RowNum > ' + CAST(@Offset AS NVARCHAR(15)) + '
        AND RowNum <= ' + CAST(@Offset + @RowsOfPage AS NVARCHAR(15));
