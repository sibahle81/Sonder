...
Create PROCEDURE [claim].[ClaimsPendingDiseaseReport]
	@DateFrom As DATE,
	@DateTo AS DATE
AS
BEGIN	
	SELECT  DISTINCT C.[ClaimId]											[Id],
			...
			PLO.FirstName [MainMemberSurname]
			
			FROM [claim].[Event] (NOLOCK) E 
			INNER JOIN [claim].[PersonEvent] (NOLOCK) PE ON E.EventId = PE.EventId
			INNER JOIN [claim].[PersonEventDiseaseDetail] (NOLOCK) PEDD ON PE.PersonEventId = PEDD.PersonEventId
			INNER JOIN [claim].[DiseaseType] (NOLOCK) DT ON PEDD.TypeOfDiseaseId = DT.DiseaseTypeID
			...
			WHERE E.CreatedDate BETWEEN '2022-06-06' AND '2023-06-06' 
