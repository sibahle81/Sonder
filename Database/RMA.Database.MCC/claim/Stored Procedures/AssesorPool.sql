...
-- =========================EXECUTE==================
-- EXEC	[claim].[AssesorPool] 1, 10, 'DateCreated', 'DESC', ''
-- ==================================================

CREATE PROCEDURE [claim].[AssesorPool] 
(
	@PageNumber		AS	INT,
	@RowsOfPage		AS	INT,
...
BEGIN
		...
		BEGIN -- DISEASE CONDITIONS
			...
			SET @SelectDiseases = 
					'SELECT  
							...
							CASE 
								WHEN cl.[ClaimReferenceNumber] LIKE '''+ @ClaimRef + ''' THEN '''+ @DescriptionA + ''' + cl.[ClaimReferenceNumber]
								ELSE ''' + @DescriptionB + ''' + cl.[ClaimReferenceNumber]
							END							AS [Description],
	...
	BEGIN -- EXECUTE
		...
		INSERT INTO #AssesorPool EXEC SP_EXECUTESQL @SelectDiseases
		INSERT INTO #AssesorPool EXEC SP_EXECUTESQL @SelectAccidents
		
		DECLARE @FinalQuery AS NVARCHAR(MAX)
		SET		@FinalQuery = 'SELECT  * FROM #AssesorPool '  + @orderQuery + '
	...
	BEGIN -- CLEAN UP
		...
		IF OBJECT_ID('tempdb..#AssesorPool') IS NOT NULL DROP TABLE #AssesorPool;
