CREATE Procedure [claim].[GetAutoAdjudicateClaims]
(
...
exec [claim].[GetAutoAdjudicateClaims] 0
...
BEGIN
	...
	IF(@isNotificationOnly = 0)
		Begin
			...
			Select 
				...
				where DK.KeyName = 'PersonEvent' and KeyValue = CAST(PE.PersonEventId as varchar)  and D.DocumentStatusId = 6 And DSDT.DocumentSetId LIKE
			...
			AND CBC.ClaimBucketClassId != 15 AND PE.SuspiciousTransactionStatusId = 2 AND C.ClaimStatusId  IN (3,15,37,38,39,1) 
			...
			AND PE.CompCarePersonEventId is Not null AND PER.IsVopdVerified = 1 AND CP.IsStraightThroughProcess IS NULL
			AND CBC.ClaimBucketClassId != 15 AND PE.SuspiciousTransactionStatusId = 2 AND C.ClaimStatusId IN (3,15,37,38,39,1) 
	...
	BEGIN
		...
		Where pe.IsStraightThroughProcess = 1 AND CBC.IsStraightThroughProcessBenefit = 1 AND PR.ProductClassId = 1 
		AND CBC.ClaimBucketClassId = 15 AND PE.SuspiciousTransactionStatusId = 2 AND C.ClaimStatusId NOT IN (40,41)
