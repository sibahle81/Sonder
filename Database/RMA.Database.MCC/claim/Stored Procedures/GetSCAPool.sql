CREATE PROCEDURE [claim].[GetSCAPool] 
(
	@PageNumber			AS	INT,
	@RowsOfPage			AS	INT,
...
BEGIN
	...
	BEGIN
		...
		DECLARE	@Reopened INT = (SELECT claimStatusid FROM CLAIM.ClaimStatus WHERE [name] = 'Reopened');
	...
	IF(@ReAllocate = 1 AND @SearchCreatia != 'Unassigned')
		...
		SELECT
				...
				AND CL.ClaimStatusId in (@autoAcknowledged,  @manualAcknowledged,  @pendingInvestigations, @Reopened)
				AND PE.IsStraightThroughProcess = 0
				AND PE.PersonEventBucketClassId != 1
				AND cl.claimLiabilityStatusId in (@Pending)
