...
CREATE PROCEDURE [claim].[PaymentReversalReport]
	@StartDate		As Date,
	@EndDate		AS Date,
...
IF OBJECT_ID('[claim].[PaymentReversalTest]', 'U') IS NOT NULL
DROP TABLE [claim].[PaymentReversalTest];

SELECT DISTINCT TOP 1000
trns.TransactionId				as 'Transaction_ID'
,clm.ClaimReferenceNumber		as 'Claim_Number'
,b.[Name]						as 'Branch'
,py.CreatedBy					as 'Requested_reversal'
,py.payee						as 'Debtor_created'
,trns.REASON					as 'Reversal_Reason' ---Business will provide
,trns.Amount					as 'Reversal_amount'
,trns.TransactionDate			as 'Date'
,pc.[NAME]						as 'Class'
,prd.[Name]	      				as 'Product'
,prd.Id							as 'ProductId'
,py.CreatedBy					as 'Authorised_By'
INTO [claim].[PaymentReversalTest]
...
--inner JOIN [security].[User] (NOLOCK) u ON py.EmailAddress = u.Email
inner JOIN [common].[Branch] (NOLOCK) b ON py.Branch = b.Id
WHERE py.Branch <> 'UNIVERSAL'
and trns.REASON is not null
