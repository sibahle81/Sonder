...
---- Description:	Report Process [claim].[ClaimSummaryPendingVolumesReport] 
---- =============================================


 CREATE PROCEDURE [claim].[ClaimSummaryPendingVolumesReport]
	...
	DROP TABLE #tempfinal;
	 ...
	 SELECT DISTINCT 
		...
		END AS [Business Class],
			...
			CAST(pe.[DateCaptured] AS DATE)		[Date Captured],
			...
			LEFT JOIN [claim].[ClaimsCalculatedAmount] cca ON cca.ClaimId = clm.ClaimId
			LEFT JOIN [Payment].[Payment] (NOLOCK) pay ON pay.claimid = clm.claimid
...
IF OBJECT_ID(N'tempdb..#final', N'U') IS NOT NULL
	DROP TABLE #final;
		 	...
		 	SELECT  top 10
					...
					Sum([PendingClaimsAmount]) AS [PendingClaimsAmount],
					Min([Date Captured]) AS [Date Captured]
			INTO #final
			FROM #tempfinal
...
IF OBJECT_ID(N'tempdb..#final1', N'U') IS NOT NULL
	DROP TABLE #final1;
...
IF OBJECT_ID(N'tempdb..#final2', N'U') IS NOT NULL
	DROP TABLE #final2;
...
IF OBJECT_ID(N'tempdb..#final3', N'U') IS NOT NULL
	DROP TABLE #final3;			
...
IF OBJECT_ID(N'tempdb..#final4', N'U') IS NOT NULL
	DROP TABLE #final4;
...
IF OBJECT_ID(N'tempdb..#final5', N'U') IS NOT NULL
	DROP TABLE #final5;
