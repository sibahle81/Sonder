CREATE PROCEDURE [claim].[GetDiseaseClaimsToAutoAcknowledge]
AS 
BEGIN
	...
	FROM Claim.PersonEvent AS PE 
		...
		WHERE E.EventTypeId IN (3) AND e.IsDeleted = 0 AND PO.[Code] IS NOT NULL --AND LEN(PO.[name]) = 3
		AND C.ClaimStatusId in (1,2,15,36)	-- 1: New, 2: Received, 15: ReOpened, 36: Submitted
		--AND PE. PersonEventId in (23024467)
