CREATE PROCEDURE [payment].[PaymentAuditLogDelete]
	@DateFrom As Date,
	@DateTo AS Date
...
BEGIN
/*
EXEC [payment].[PaymentAuditLogDelete] @DateFrom = '2020-01-01', @DateTo = '2022-12-31'
*/
 Declare  @PaymentResults Table(
  ...
  select  json_value(al.NewItem,'$.Id') as itemId, 
	...
	where al.itemtype = 'payment_payment' 
		and al.[date] >= @DateFrom and  al.[date] <= @DateTo
	...
	while(@index <= @total)
		begin
			...
			select top 1 itemId, @policyId, username,'Approved','Claim', @Amount, @reference, 
					@claimNumber, @product, null, @AccountNo, [date],null,null,null,null,null,null,null,-1
			from [audit].[AuditLog] (NOLOCK) 
			...
			select top 1 itemId, @policyId, username,'Authorised','Claim', @Amount, @reference, 
					@claimNumber, @product, null, @AccountNo, null,[date],null,null,null,null,null,null,0 
			from [audit].[AuditLog] (NOLOCK) 
				where json_value(newitem,'$.PolicyId') = @policyId
					and ItemId != 0 and itemtype='claim_Claim'
			...
			insert into @Results
				select top 1 itemId, @policyId, Reciepients,'letterSent','Claim', @Amount, @reference, 
					@claimNumber, @product, null, @AccountNo, null,null,null,null,null,null,null,createddate,3
				from [campaign].[EmailAudit](NOLOCK) ea
				where itemtype = 'payment'
					and ea.subject = 'payment confirmation'
					and ea.issuccess = 1 

			if Not Exists(Select ItemId From @Results Where PolicyId = @PolicyId and PaymentStatus = 'letterSent')
				insert into @Results
					VALUES (NULL, @policyId, null,'letterSent','Claim', @Amount, @reference, 
						@claimNumber, @product, null, @AccountNo, null,null,null,null,null,Null,Null, null,3)
