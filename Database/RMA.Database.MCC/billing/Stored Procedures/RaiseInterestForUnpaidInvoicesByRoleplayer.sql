...
-- =============================================
CREATE PROCEDURE [billing].[RaiseInterestForUnpaidInvoicesByRoleplayer] 
	@roleplayer int,
	@dateRange datetime
AS
BEGIN
	...
	and t2.TransactionTypeId = 7) and cf.RolePlayerId = @roleplayer --dont double charge interest in the same month/quarter etc
			...
			declare @currentInvoicePrimeRateId int = (select top 1 id from @thisInvoicePrimeRates)
					while (select count(id) from @thisInvoicePrimeRates) > 0
						...
						id = @currentInvoicePrimeRateId)
							...
							if 	@invoicePrimeRateStartDate <= @monthInterestIsRaisedFor and @invoicePrimeEndDate >  @monthInterestIsRaisedForEnding
							begin	
								set	@numberOfInterestDays = DATEDIFF(day,@monthInterestIsRaisedFor, @monthInterestIsRaisedForEnding) +1 -- fix off by 1 day error	
							end

							if 	@invoicePrimeRateStartDate > @monthInterestIsRaisedFor and @invoicePrimeEndDate >  @monthInterestIsRaisedForEnding
							begin	
								set	@numberOfInterestDays = DATEDIFF(day,@invoicePrimeRateStartDate, @monthInterestIsRaisedForEnding) +1 -- fix off by 1 day error
							end
