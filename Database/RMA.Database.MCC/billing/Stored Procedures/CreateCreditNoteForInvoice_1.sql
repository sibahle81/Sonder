...
CREATE PROCEDURE [billing].[CreateCreditNoteForInvoice]
/* =============================================
	...
	BEGIN TRAN trxCreditNoteForInvoice
		BEGIN TRY
			...
			BEGIN 
		        ...
		        DECLARE @NextID INT = (SELECT NextNumber FROM Common.DocumentNumbers d (NOLOCK) WHERE   d.Name = 'RolePlayerId')
				
				SELECT @NextID = @NextID  + 1
				
				SELECT @creditNoteReferenceNumber = 'CN' +  CONVERT(VARCHAR(12), GETDATE(), 112) + '0' +  CONVERT(VARCHAR(12), @NextID)
				...
				UPDATE  Common.DocumentNumbers WITH (UPDLOCK) SET NextNumber = @NextID WHERE   Name = 'RolePlayerId'
