...
begin
	...
	select  i.InvoiceNumber, t.TransactionId, dbo.GetTransactionBalanceForInterest(t.TransactionId),i.invoiceid, cf.RolePlayerId, i.InvoiceDate  from billing.invoice i 
	join billing.transactions t on 
	t.invoiceid = i.invoiceid
	join policy.Policy p on p.PolicyId = i.PolicyId
	join client.FinPayee cf on cf.RolePlayerId = p.PolicyOwnerId
	...
	and p.ProductOptionId in (select id from product.ProductOption where productid =@productId)
	...
	insert into  @primeRates
	 select [value],startDate,endDate from common.primerate where (startDate between  @minInterestMonth and @maxInterestMonthEndDate)
	 or (endDate  between  @minInterestMonth and @maxInterestMonthEndDate)
	 order by startdate 
	...
	begin	
			...
			insert into billing.[Transactions] (
					  ...
					  values (@roleplayerId,1,@summedInterest,@postingDate,7,Getdate(),Getdate(),'system@randmutual.co.za',	'system@randmutual.co.za',@linkedTransactionId, concat('Overdue-Invoice:', @invoiceNumber))
