CREATE PROCEDURE [billing].[CreateCreditNotesForPolicyWithClaim]
@policyId int,
@claimId int
AS BEGIN
...
DECLARE @InvoicesToBeAdjustedTotals table(PolicyId int, PolicyOwnerId int,
InvoiceId int, InvoiceNumber varchar(50), InvoiceDate date, CollectionDate date, TotalPremiumContribution decimal(18,2), InvoiceAmount decimal(18,2));
insert into @InvoicesToBeAdjustedTotals(PolicyId, PolicyOwnerId, InvoiceId,  InvoiceNumber, InvoiceDate, CollectionDate,
TotalPremiumContribution, InvoiceAmount)
(select PolicyId,PolicyOwnerId,InvoiceId,InvoiceNumber,InvoiceDate,CollectionDate,
sum(premiumcontribution),InvoiceAmount from
@InvoicesToBeAdjusted
group by PolicyId,PolicyOwnerId,InvoiceId,InvoiceNumber,InvoiceDate,CollectionDate,InvoiceAmount)
...
-- Create credit notes for incorrect unpaid invoices
  ...
  Amount, TransactionDate, BankReference, TransactionTypeId, CreatedDate, ModifiedDate, CreatedBy, ModifiedBy, RmaReference)
  (select distinct InvoiceId, PolicyOwnerId, null, 2, InvoiceAmount, EOMONTH(InvoiceDate), '',
   4, (select getdate()), (select getdate()), 'system@randmutual.co.za', 'system@randmutual.co.za',
...
-- Settle incorrect unpaid invoices
	...
	select distinct p.PolicyId, i.CollectionDate, p.InstallmentPremium, 3, '', i.InvoiceDate,
	 'system@randmutual.co.za', 'system@randmutual.co.za', (select getdate()), (select getdate())
	from @InvoicesToBeAdjustedTotals i, policy.Policy p
	where TotalPremiumContribution > 0 and p.PolicyId = i.PolicyId
	...
	select i.InvoiceId, p.PolicyOwnerId, 1, p.InstallmentPremium, i.InvoiceDate, p.PolicyNumber, 6,
	'system@randmutual.co.za', 'system@randmutual.co.za', (select getdate()), (select getdate())
	...
	and exists (select RolePlayerId from client.FinPayee where RolePlayerId =  p.PolicyOwnerId)
