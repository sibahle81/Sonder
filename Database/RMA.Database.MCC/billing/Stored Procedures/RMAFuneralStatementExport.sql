...
CREATE PROCEDURE [billing].[RMAFuneralStatementExport] 
	@invoiceId int
AS
BEGIN
...
AND EXISTS (SELECT p.* FROM [common].[Period] p WHERE p.StartDate >= Transactions.TransactionDate and p.[Status] != 'Future') 
...
SELECT   transactions.transactiondate, 
				...
				THEN transactions.RmaReference ELSE transactions.BankReference END, 
				CONCAT('Payment - ', 
                (SELECT DATENAME(month,ISNULL(p.StartDate, Transactions.TransactionDate)) FROM [common].[Period] p WHERE transactions.transactiondate >= p.StartDate
				AND transactions.transactiondate <= p.EndDate)) AS [description],
                0 AS DebitAmount, 
...
FROM   [billing].[transactions] Transactions 
       ...
       INNER JOIN [common].[transactiontype] TransactionType 
               ON transactions.transactiontypeid = transactiontype.id 
WHERE  RolePlayer.RolePlayerId = @rolePlayerId
AND Transactions.TransactionTypeId = 8 -- invoice
