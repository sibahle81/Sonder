/* =============================================
Name:			Sp_billing_ProcessSatgingTransaction
...
Culprits:		
============================================= */
CREATE PROCEDURE [billing].[ProcessStagedBankstatementImport] 
AS
BEGIN
	Declare @bankStatemententrystaging table([BankStatementEntryId] int,
		[IsActive] varchar(50),
        ...
        Declare @currentId int =(SELECT TOP 1 BankStatementEntryId FROM @bankStatemententrystaging)
        ...
        BEGIN
			BEGIN TRY  
                BEGIN TRAN tnx_main
                    ...
                    ,cast([User] as varchar(100)),cast([StatementAndLineNumber] as varchar(30)),1,0,0,'BackendProcess',GETDATE(),'BackendProcess',GETDATE() from @bankStatemententrystaging where BankStatementEntryId = @currentId
                           
                    COMMIT TRAN tnx_main
                    UPDATE [Load].[bankStatemententrystaging] SET ErrorMessage = 'success' , Imported = 1 where BankStatementEntryId = @currentId
                ...
                END TRY  
					BEGIN CATCH  
						ROLLBACK TRAN tnx_main
							DELETE FROM @bankStatemententrystaging where BankStatementEntryId = @currentId
                            SET @currentId  =(SELECT top 1 BankStatementEntryId from @bankStatemententrystaging)
                            UPDATE [Load].[bankStatemententrystaging] SET ErrorMessage = concat('error-',(SELECT ERROR_MESSAGE())), Imported = 2 where BankStatementEntryId = @currentId
                    END CATCH
                END
